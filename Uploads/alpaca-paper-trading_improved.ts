...
import { MarketData, OptionsChain, Position, TradeSignal } from './types';
...
export class AlpacaPaperTrading extends EventEmitter {
  ...
  private is0DTE(expiration: Date): boolean {
    const now = new Date();
  ...
  private async findOptionContract(underlying: string, signal: TradeSignal): Promise<string | null> {
    ...
    const todayChains = chains.filter(opt => this.is0DTE(new Date(opt.expiration)));
