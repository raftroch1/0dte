...
import { MarketData, OptionsChain, TradeSignal, Strategy, DailyMetrics } from './types';
import { AlpacaPaperTrading, PaperTradingConfig } from './alpaca-paper-trading_improved';
import { GreeksSimulator, OptionPricingInputs } from './greeks-simulator';
...
export class BacktestingEngine extends EventEmitter {
  ...
  private recordDailyMetrics(): void {
    ...
    const dailyMetric: DailyMetrics = {
      ...
      averageHoldTime: this.calculateAverageHoldTime(metrics.todayTrades),
      morningTrades: metrics.todayTrades.filter(t => t.timestamp.getHours() < 12).length,
      afternoonTrades: metrics.todayTrades.filter(t => t.timestamp.getHours() >= 12).length,
      momentumTrades: 0, // Would need to track this based on signal type
      regimeTrades: 0, // Would need to track this based on signal type
      maxRiskTaken: 0, // Would need to calculate based on position sizes
      riskAdjustedReturn: metrics.dayPnL / Math.max(metrics.maxDrawdown, 0.01),
