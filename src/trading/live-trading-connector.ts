...
import { AlpacaPaperTrading } from './alpaca-paper-trading';
import { AdaptiveStrategySelector } from '../strategies/adaptive-strategy-selector';
import { MarketRegimeDetector } from '../data/market-regime-detector';
import { TechnicalAnalysis } from '../data/technical-indicators';
import { MarketData, OptionsChain, TradeSignal, Position, Strategy, AccountConfig } from '../utils/types';
...
export class LiveTradingConnector extends EventEmitter {
  ...
  private async generateTradingSignal(): Promise<TradeSignal | null> {
    ...
    try {
      ...
      const regime = MarketRegimeDetector.detectRegime(this.marketDataBuffer);
      
      // Calculate technical indicators
      const indicators = TechnicalAnalysis.calculateAllIndicators(this.marketDataBuffer);
